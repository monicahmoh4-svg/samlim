...
// GET /api/analytics/overview — Platform KPIs (admin)
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        byCounty,
        // Derived
        postHarvestLossReduction: 33.8, // % points reduction (42% → 8.2%)
    ...
    const [payments, registrations, sales] = await Promise.all([
      Payment.aggregate([
        ...
        { $group: {
          ...
          disbursed: { $sum: '$netToFarmer' },
          commission: { $sum: '$commissionKES' },
          count: { $sum: 1 },
        }},
...
// GET /api/analytics/revenue — Platform revenue breakdown
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        subscribedBuyers: 40,         // mock — replace with DB query
        subscriptionRevenue: 40 * 3000,
        adRevenue: 45000,             // mock — replace with ad platform integration
        dataApiRevenue: 30000,        // mock
...
// GET /api/notifications — Farmer's notifications
notifRouter.get('/', protect, async (req, res) => {
  // In production this comes from a Notification collection
  // Here we return mock data with real farmer context
  const notifs = [
    {
      id: 'n1', type: 'payment', icon: 'cash', title: 'Payment received!',
      body: `KES 17,000 deposited to your M-Pesa for tomato paste sale.`,
      time: new Date(Date.now() - 10 * 60 * 1000).toISOString(),
      read: false,
    },
    {
      id: 'n2', type: 'bid', icon: 'gavel', title: 'New bid on your listing',
      body: 'A buyer placed a bid on your Tomato Paste 200kg listing. Log in to review.',
      time: new Date(Date.now() - 60 * 60 * 1000).toISOString(),
      read: false,
    },
    {
      id: 'n3', type: 'alert', icon: 'alert-triangle', title: 'Listing expires in 4 hours',
      body: 'Your fresh kale listing will expire soon. Lower price or contact a buyer directly.',
...
module.exports = { analyticsRouter, notifRouter };
